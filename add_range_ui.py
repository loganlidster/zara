...
# Find the filters section and add range controls after it
# Look for the section with Symbol, Method, Session filters

range_ui = '''
          {/* Mode Selection */}
          <div className="col-span-full mb-4 pb-4 border-b border-gray-200">
...
# Insert after the Session filter
insert_point = '            </div>\n          </div>\n\n          <button'
if insert_point in content:
    content = content.replace(insert_point, '            </div>\n          </div>\n' + range_ui + '\n          <button')
    with open('frontend-dashboard/app/reports/best-performers/page.tsx', 'w') as f:
        f.write(content)
    print("Added range testing UI controls")
else:
